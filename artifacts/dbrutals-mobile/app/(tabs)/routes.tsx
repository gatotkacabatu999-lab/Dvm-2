...
export default function RoutesScreen() {
  ...
  type Section = { title: string; data: Route[] };
  const sections: Section[] = [
    ...(amRoutes.length > 0 ? [{ title: "AM Shift", data: amRoutes }] : []),
    ...(pmRoutes.length > 0 ? [{ title: "PM Shift", data: pmRoutes }] : []),
    ...(otherRoutes.length > 0 ? [{ title: "Other", data: otherRoutes }] : []),
  ];

  const flatItems: Array<{ type: "header"; title: string } | { type: "route"; route: Route }> = [];
  for (const section of sections) {
    flatItems.push({ type: "header", title: section.title });
    for (const route of section.data) {
      flatItems.push({ type: "route", route });
    }
  }
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      {flatItems.length === 0 ? (
        <EmptyState
          ...
          subtitle={search ? "Try a different search" : "Routes will appear here once added"}
        />
      ) : (
        <FlatList
          data={flatItems}
          keyExtractor={(item, i) => (item.type === "header" ? `h-${item.title}` : `r-${item.route.id}-${i}`)}
...
const styles = StyleSheet.create({
  ...
  screenTitle: { fontSize: 28, fontFamily: "Inter_700Bold", letterSpacing: -0.5, marginBottom: 12 },
  searchBar: { flexDirection: "row", alignItems: "center", gap: 10, paddingHorizontal: 12, paddingVertical: 10, borderWidth: 1 },
  ...
  pointsBadge: { flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: 8, paddingVertical: 4 },
